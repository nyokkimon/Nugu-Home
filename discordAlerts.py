...
def getHelpEmbed():
    ...
    embed.set_footer(text="ng-home: Who is on my network?")
    return embed
...
def formatScanHeader(total_active, total_registered, frequency, history):
    ...
    return (
        f"📡 **ng-home: Presence Check Initiated**\n"
        f"```status\n"
...
def formatNuguAlert(device):
    ...
    return (
        f"⚠️ **ng-home: NEW DEVICE DETECTED!**\n"
        f"```yaml\n"
        f"IP:       {device['ip']}\n"
        f"MAC:      {device['mac']}\n"
...
def formatKnownDevice(device):
    ...
    now = datetime.now().strftime("%H:%M:%S")
    return (
        f"✅ **ng-home: {device['name']} is online**\n"
        f"```yaml\n"
        f"Status:   Recognized\n"
        f"IP:       {device['ip']}\n"
        f"MAC:      {device['mac']}\n"
